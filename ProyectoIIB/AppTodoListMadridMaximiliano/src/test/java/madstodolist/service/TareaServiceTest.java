...
public class TareaServiceTest {
    ...
    public void testGenerarCSV() {
        ...
        // THEN
        assertThat(csv).contains("ID,Título");
    ...
    public void testMarcarTareaComoCompletada() {
        ...
        TareaData tareaCompletada = tareaService.marcarCompletada(tareaId, true);

        // THEN
        // la tarea está marcada como completada
        assertThat(tareaCompletada).isNotNull();
        assertThat(tareaCompletada.getCompletada()).isTrue();

        // y si la recuperamos de la base de datos, sigue estando completada
        TareaData tareaBD = tareaService.findById(tareaId);
        assertThat(tareaBD.getCompletada()).isTrue();
    }

    @Test
    public void testDesmarcarTareaCompletada() {
        // GIVEN
        // Un usuario y una tarea completada en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long tareaId = ids.get("tareaId");
        tareaService.marcarCompletada(tareaId, true);

        // WHEN
        // desmarcamos la tarea como completada
        TareaData tareaDescompletada = tareaService.marcarCompletada(tareaId, false);

        // THEN
        // la tarea ya no está marcada como completada
        assertThat(tareaDescompletada).isNotNull();
        assertThat(tareaDescompletada.getCompletada()).isFalse();

        // y si la recuperamos de la base de datos, no está completada
        TareaData tareaBD = tareaService.findById(tareaId);
        assertThat(tareaBD.getCompletada()).isFalse();
    }

    @Test
    public void marcarTareaComoCompletadaCambiaEstado() {
        // GIVEN
        // Un usuario y una tarea en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long tareaId = ids.get("tareaId");
        ...
        // y si la recuperamos de la base de datos, está completada
        TareaData tareaBD = tareaService.findById(tareaId);
        assertThat(tareaBD.getCompletada()).isTrue();
    }

    @Test
    public void marcarTareaComoNoCompletadaCambiaEstado() {
        // GIVEN
        // Un usuario y una tarea ya completada en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long tareaId = ids.get("tareaId");
        tareaService.marcarCompletada(tareaId, true);
        
        // Verificamos que la tarea está completada
